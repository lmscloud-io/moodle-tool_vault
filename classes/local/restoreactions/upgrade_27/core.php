...
use tool_vault\local\restoreactions\upgrade_27\helpers\core_cache_helper;
use tool_vault\local\restoreactions\upgrade_27\helpers\general_helper;

/**
 * Upgrade script from Moodle 2.2 to 2.7
 *
 * @package tool_vault
 * @param mixed $oldversion
 * @return void
 */
function tool_vault_27_core_upgrade($oldversion) {
    ...
    if ($oldversion < 2012030900.01) {
        ...
        general_helper::upgrade_mysql_fix_unsigned_and_lob_columns();
    ...
    if ($oldversion < 2012042300.02) {
        ...
        if ($DB->get_dbfamily() === 'mysql') {
            $sql = "DELETE cc FROM {course_completion_criteria} cc
                    ...
                    WHERE cm.id IS NULL AND cc.criteriatype = 4"; // COMPLETION_CRITERIA_TYPE_ACTIVITY
        } else {
            $sql = "DELETE FROM {course_completion_criteria}
                    ...
                    AND {course_completion_criteria}.criteriatype = 4"; // COMPLETION_CRITERIA_TYPE_ACTIVITY
        }
        $DB->execute($sql);
    ...
    if ($oldversion < 2012052900.00) {
        ...
        general_helper::upgrade_course_completion_remove_duplicates(
            'course_completions',
            array('userid', 'course'),
    ...
    if ($oldversion < 2012072401.00) {
        ...
        general_helper::upgrade_save_orphaned_questions();
    ...
    if ($oldversion < 2012101800.02) {
        ...
        general_helper::upgrade_rename_old_backup_files_using_shortname();
    ...
    if ($oldversion < 2013021800.00) {
        ...
        core_cache_helper::update_site_identifier($siteidentifier);
    ...
    if ($oldversion < 2013041900.00) {
        ...
        core_cache_helper::cache_update_default_config_stores();
        // Main savepoint reached.
    ...
    if ($oldversion < 2013072600.01) {
        general_helper::upgrade_mssql_nvarcharmax();
        general_helper::upgrade_mssql_varbinarymax();
    ...
    if ($oldversion < 2013102500.01) {
        ...
        if (empty($CFG->filesrootrecordsfixed)) {
            general_helper::upgrade_fix_missing_root_folders();
    ...
    if ($oldversion < 2013110500.01) {
        ...
        if (empty($CFG->movingmoduleupgradescriptwasrun)) {
            general_helper::upgrade_course_modules_sequences();
    ...
    if ($oldversion < 2014011701.00) {
        ...
        general_helper::upgrade_grade_item_fix_sortorder();
    ...
    if ($oldversion < 2014040800.00) {
        ...
        if ($cmcount + $sectcount > 0) {
            ...
            foreach ($rs as $rec) {
                ...
                // Update item.
                $availability = general_helper::upgrade_availability_item(0, 0,
            ...
            foreach ($rs as $rec) {
                ...
                // Update item.
                $availability = general_helper::upgrade_availability_item($rec->groupingid ? 1 : 0,
    ...
    if ($oldversion < 2014051200.06) {
        ...
        general_helper::upgrade_mimetypes($filetypes);
    ...
    if ($oldversion < 2014051202.11) {
        // Run script restoring missing folder records for draft file areas.
        general_helper::upgrade_fix_missing_root_folders_draft();

        // Main savepoint reached.
        upgrade_main_savepoint(true, 2014051202.11);
    }

    // Switch the order of the fields in the files_reference index, to improve the performance of search_references.
    if ($oldversion < 2014051203.08) {
        $table = new xmldb_table('files_reference');
        $index = new xmldb_index('uq_external_file', XMLDB_INDEX_UNIQUE, array('repositoryid', 'referencehash'));
        if ($dbman->index_exists($table, $index)) {
            $dbman->drop_index($table, $index);
        }
        upgrade_main_savepoint(true, 2014051203.08);
    }
    if ($oldversion < 2014051203.09) {
        $table = new xmldb_table('files_reference');
