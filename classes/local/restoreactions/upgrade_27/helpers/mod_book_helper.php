...
class mod_book_helper {
     ...
     * @return void
     */
    public static function mod_book_migrate_moddata_dir_to_legacy($book, $context, $path) {
        global $OUTPUT, $CFG;
        ...
        foreach ($items as $item) {
            ...
            if ($item->isFile()) {
                ...
                unset($item);  // release file handle
                self::mod_book_migrate_moddata_dir_to_legacy($book, $context, $subpath);
     ...
     * @return void
     */
    public static function mod_book_migrate_all_areas() {
        global $DB, $OUTPUT;
        ...
        foreach($rsbooks as $book) {
            ...
            self::mod_book_migrate_area($book, 'intro', 'book', $book->course, $context, 'mod_book', 'intro', 0);

            $rschapters = $DB->get_recordset('book_chapters', array('bookid'=>$book->id));
            foreach ($rschapters as $chapter) {
                self::mod_book_migrate_area($chapter, 'content', 'book_chapters', $book->course, $context, 'mod_book', 'chapter', $chapter->id);
            }
            $rschapters->close();
        }
        $rsbooks->close();
     ...
     * @return void
     */
    public static function mod_book_migrate_area($record, $field, $table, $courseid, $context, $component, $filearea, $itemid) {
        global $CFG, $DB;
