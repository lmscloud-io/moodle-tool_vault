...
class format_helper {
     ...
     * these sections.
     */
    public static function format_topics_upgrade_remove_numsections() {
        global $DB;
        ...
        foreach ($needfixing as $courseid => $numsections) {
            self::format_topics_upgrade_hide_extra_sections($courseid, $numsections);
        }

        foreach ($needsections as $courseid => $numsections) {
            self::format_topics_upgrade_add_empty_sections($courseid, $numsections);
        }
     ...
     */
    public static function format_topics_upgrade_hide_extra_sections($courseid, $numsections) {
        global $DB;
     ...
     * @param int $numsections
     */
    public static function format_topics_upgrade_add_empty_sections($courseid, $numsections) {
        global $DB;
     ...
     * these sections.
     */
    public static function format_weeks_upgrade_remove_numsections() {
        global $DB;
        ...
        foreach ($needfixing as $courseid => $numsections) {
            self::format_weeks_upgrade_hide_extra_sections($courseid, $numsections);
        }

        foreach ($needsections as $courseid => $numsections) {
            self::format_weeks_upgrade_add_empty_sections($courseid, $numsections);
        }
     ...
     * @param int $numsections
     */
    public static function format_weeks_upgrade_hide_extra_sections($courseid, $numsections) {
        global $DB;
        $sections = $DB->get_records_sql('SELECT id, name, summary, sequence, visible
            FROM {course_sections}
            WHERE course = ? AND section > ?
     ...
     * @param int $numsections
     */
    public static function format_weeks_upgrade_add_empty_sections($courseid, $numsections) {
        global $DB, $CFG;
        require_once($CFG->dirroot.'/course/lib.php'); // TODO not ideal.
        $existingsections = $DB->get_fieldset_sql('SELECT section from {course_sections} WHERE course = ?', [$courseid]);
        $newsections = array_diff(range(0, $numsections), $existingsections);
        foreach ($newsections as $sectionnum) {
            course_create_section($courseid, $sectionnum, true);
