...
use tool_vault\local\restoreactions\upgrade_31\helpers\general_helper;

/**
 * Upgrade core 2.7-3.1
 *
 * @package tool_vault
 * @param float $oldversion
 * @return bool
 */
function tool_vault_31_core_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2014082900.02) {
        ...
        if ($CFG->enablegroupmembersonly) {
            ...
            foreach ($rs as $cm) {
                ...
                $availability = general_helper::upgrade_group_members_only($cm->groupingid, $cm->availability);
    ...
    if ($oldversion < 2015060400.02) {
        ...
        if (empty($CFG->upgrade_minmaxgradestepignored)) {
            ...
            general_helper::upgrade_minmaxgrade();
    ...
    if ($oldversion < 2015061900.00) {
        ...
        if (empty($CFG->upgrade_extracreditweightsstepignored)) {
            general_helper::upgrade_extra_credit_weightoverride();
    ...
    if ($oldversion < 2015062500.01) {
        ...
        if (empty($CFG->upgrade_calculatedgradeitemsignored)) {
            general_helper::upgrade_calculated_grade_items();
    ...
    if ($oldversion < 2015090801.00) {
        ...
        general_helper::upgrade_course_tags();
    ...
    if ($oldversion < 2016051300.00) {
        ...
        general_helper::make_competence_scale();
    ...
    if ($oldversion < 2016051700.01) {
        ...
        if (empty($CFG->upgrade_letterboundarycourses)) {
            ...
            // This is here to freeze the gradebook in affected courses.
            general_helper::upgrade_course_letter_boundary();
